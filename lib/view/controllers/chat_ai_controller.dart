...
class ChatAIController extends GetxController {
  ...
  setupModel() {
    final apiKey = Constants.googleAIAPIKey;
    fdLog.title(
      'Chat AI Controller - setupModel',
      'API Key loaded: ${apiKey.isNotEmpty ? "YES (${apiKey.length} chars)" : "NO - EMPTY!"}',
    );
    
    if (apiKey.isEmpty) {
      fdLog.title(
        'Chat AI Controller - setupModel',
        'API Key is empty! Please check your .env file',
      );
      return;
    }
